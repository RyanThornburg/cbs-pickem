...
class EspnClient:
    def get_scoreboard(self) -> dict[str, Any]:
        """every NFL game for the current week, live status/situation included"""
        return self._fetch(API_URL).json()
...
def get_scoreboard() -> Scoreboard:
    """current week's NFL scoreboard"""
    return fetch_and_validate_one(
        "ESPN scoreboard", lambda: EspnClient().get_scoreboard(), Scoreboard
    )
